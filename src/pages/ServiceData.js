// src/data/servicesData.js
import pvcWall from "../assets/images/usmania/h2.jpg";
import ceiling from "../assets/images/usmania/u19.jpg";
import charcoal from "../assets/images/usmania/u13.jpg";
import wps from "../assets/images/usmania/u11.jpg";
import fluted from "../assets/images/usmania/u18.jpg";
import pvcDoor from "../assets/images/usmania/door.jpg";
import uvMarble from "../assets/images/usmania/u8.jpg";
import wallpaper from "../assets/images/usmania/u2.jpg";

const ServiceData = {
  topHeading: "Our Premium Interior Solutions",
  bigQuote: "“Transforming ordinary spaces into timeless masterpieces.”",

  services: [
    {
      title: "PVC Wall Panel",
      slug: "pvc-wall-panel",
      image: pvcWall,
      heading: "PVC Wall Panels – Elegance Meets Durability",
      description: `
        Our premium PVC wall panels redefine luxury living. Crafted with precision, these panels combine elegance, durability, and effortless maintenance, making them the perfect choice for modern homes and offices. Available in a wide array of textures and finishes, they provide unmatched style while resisting moisture, termites, and wear. 

        At Usmani Interior, we don’t just cover walls — we elevate them. Every panel is designed to complement your lifestyle and architectural vision. 

        Why choose Usmani Interior? We blend innovation with craftsmanship, ensuring each installation reflects sophistication and long-lasting beauty. Our commitment: transforming your walls into statements of class and comfort.
      `,
    },
    {
      title: "Ceiling Panel",
      slug: "ceiling-panel",
      image: ceiling,
      heading: "Ceiling Panels – A Touch of Grandeur",
      description: `
        Transform ordinary ceilings into breathtaking designs with our premium ceiling panels. They offer insulation, acoustic control, and visually stunning patterns that define luxury interiors. From minimalist modern to classic opulence, each panel enhances ambiance and sets the tone for your space.

        Why always Usmani Interior? Our expert designers ensure every ceiling is a masterpiece, meticulously installed with attention to detail. Our commitment is simple: we create ceilings that inspire admiration, lasting comfort, and aesthetic excellence.
      `,
    },
    {
      title: "Charcoal Panel",
      slug: "charcoal-panel",
      image: charcoal,
      heading: "Charcoal Panels – Luxury in Every Line",
      description: `
        Charcoal panels bring bold character and sophisticated texture to your interiors. With a premium matte finish and high-end appeal, they are perfect for accent walls, living rooms, and commercial spaces seeking elegance with impact. 

        Why choose Usmani Interior? Our panels combine style with durability, installed by craftsmen passionate about perfection. Our commitment: transforming every space into a refined environment that reflects your taste and our expertise.
      `,
    },
    {
      title: "WPS Louvers",
      slug: "wps-louvers",
      image: wps,
      heading: "WPS Louvers – Redefining Modern Architecture",
      description: `
        WPS Louvers represent contemporary design at its finest. Lightweight, stylish, and versatile, they add depth, airflow, and aesthetic appeal to any wall or ceiling. Ideal for homes, offices, or commercial interiors, these louvers balance function with elegance. 

        Why always Usmani Interior? We ensure every louver is aligned to perfection and seamlessly integrated. Our commitment: to provide modern, sophisticated interiors that enhance lifestyle and architectural beauty.
      `,
    },
    {
      title: "Fluted Panel",
      slug: "fluted-panel",
      image: fluted,
      heading: "Fluted Panels – Timeless Elegance",
      description: `
        Fluted panels are the epitome of elegance, offering vertical grooves that add rhythm, texture, and depth to interiors. Perfect for feature walls, headboards, and office receptions, these panels reflect subtle sophistication and timeless style. 

        Why Usmani Interior? Our design philosophy merges creativity with precision. Each fluted panel is installed to perfection, and our commitment is to deliver interiors that resonate with luxury, harmony, and excellence.
      `,
    },
    {
      title: "PVC Door",
      slug: "pvc-door",
      image: pvcDoor,
      heading: "PVC Doors – Strong, Stylish, Sustainable",
      description: `
        Our PVC doors are designed for elegance, strength, and long-lasting performance. Lightweight, durable, and resistant to wear and environmental factors, they are suitable for homes, offices, and premium commercial spaces. Available in multiple finishes, they enhance every interior with modern sophistication. 

        Why choose Usmani Interior? Every door is crafted and installed with meticulous attention to detail. Our commitment: delivering functional luxury, unmatched durability, and timeless design for your space.
      `,
    },
    {
      title: "UV Marble Sheet",
      slug: "uv-marble-sheet",
      image: uvMarble,
      heading: "UV Marble Sheets – Luxury Redefined",
      description: `
        Experience the grandeur of marble without the weight and cost. Our UV marble sheets are high-gloss, scratch-resistant, and available in exquisite patterns, offering a royal touch to walls and surfaces. Easy to install, durable, and elegant, they transform interiors into luxurious environments. 

        Why always Usmani Interior? We select only premium quality materials and expert craftsmanship for flawless finishes. Our commitment: creating a luxurious aesthetic with long-lasting functionality.
      `,
    },
    {
      title: "Wallpaper",
      slug: "wallpaper",
      image: wallpaper,
      heading: "Wallpaper – Design Beyond Imagination",
      description: `
        Wallpapers are more than coverings; they are canvases for expression. From modern minimalistic textures to luxurious golden motifs, our collection turns walls into works of art, creating spaces that inspire and impress. 

        Why choose Usmani Interior? We craft wallpaper designs that blend creativity, quality, and style. Our commitment: to elevate your interiors with bespoke designs that reflect both your personality and our craftsmanship.
      `,
    },
  ],
};

export default ServiceData;
